...
京东京喜工厂
更新时间：2021-6-7
修改时间：2021-6-17 注释内置助力码；屏蔽三个获得tuanid的地址
修改时间：2021-6-19 设置第一个账号助力三个助力码，不设置，第一次运行没助力码split会报错终止
...
const inviteCodes = ['jItA_nKGdgOPCIfji2bGcg==@yowvwGg9reHcZugGzgBTiQ==@IVmDIuSc2aP-3wcrPx-Djg==@FZFsLzStTJiYnwnDqaanKA=='];
...
async function joinLeaderTuan() {
  ...
  let res = await updateTuanIdsCDN(), res2 = await updateTuanIdsCDN("http://127.0.0.1")
  if (!res) res = await updateTuanIdsCDN('https://127.0.0.1');
...
function updateTuanIdsCDN(url = 'https://127.0.0.1') {
  return new Promise(async resolve => {
    const options = {
      ...
      if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
        ...
        if (!$.tuanConfigs) {
          // await updateTuanIdsCDN('https://cdn.jsdelivr.net/gh/gitupdate/updateTeam@master/shareCodes/jd_updateFactoryTuanId.json');
			await updateTuanIdsCDN('https://127.0.0.1');
          if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
            tuanActiveId = $.tuanConfigs['tuanActiveId'];
            console.log(`拼团活动ID: 获取成功 ${tuanActiveId}\n`)
          } else {
